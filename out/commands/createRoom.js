...
async function createRoom(context) {
    try {
        ...
        if (!ablyApiKey) {
            const action = await vscode.window.showWarningMessage('Ably API key not found. You need an API key to create contest rooms.', 'Enter API Key', 'Get API Key', 'Cancel');
            ...
            if (action === 'Enter API Key') {
                ...
                try {
                    await vscode.workspace.fs.writeFile(envPath, Buffer.from(`ABLY_API_KEY=${ablyApiKey}\n`));
        ...
        const numProblems = await vscode.window.showQuickPick(['1', '2', '3', '4', '5'], {
        ...
        const timeLimitStr = await vscode.window.showQuickPick(['30 minutes', '60 minutes', '90 minutes', '120 minutes', 'Custom'], {
        ...
        try {
            ...
            vscode.window.showInformationMessage(`Contest room created! Room Code: ${roomId}`, 'Copy Room Code').then(action => {
