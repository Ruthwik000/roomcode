...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('dsaRoom.createRoom', async () => {
        ...
        if (currentRoom && currentRoom.questions.length > 0) {
            ...
            const action = await vscode.window.showInformationMessage(`Room created! Code: ${currentRoom.id}`, 'Start Contest Now', 'Copy Room Code', 'Cancel');
            if (action === 'Copy Room Code') {
                ...
                const startAction = await vscode.window.showInformationMessage('Ready to start the contest?', 'Start Contest', 'Wait');
    ...
    context.subscriptions.push(vscode.commands.registerCommand('dsaRoom.joinRoom', async () => {
        ...
        if (currentRoom && currentRoom.questions.length > 0) {
            currentQuestion = currentRoom.questions[0];
            vscode.window.showInformationMessage(`Joined contest with ${currentRoom.questions.length} questions. Waiting for host to start...`);
    ...
    context.subscriptions.push(vscode.commands.registerCommand('dsaRoom.startContest', () => {
        if (!currentRoom) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('dsaRoom.fetchRandomProblem', async () => {
        try {
            ...
            if (!workspaceFolder) {
                const action = await vscode.window.showErrorMessage('No folder is open. Please open a folder first (File > Open Folder)', 'Help');
                if (action === 'Help') {
            ...
            await fs.writeFile(path.join(problemFolder, 'problem.json'), JSON.stringify(problemData, null, 2));
            await fs.writeFile(path.join(problemFolder, 'testcases.json'), JSON.stringify(problemData.testCases, null, 2));
    ...
    context.subscriptions.push(vscode.commands.registerCommand('dsaRoom.fetchProblem', async () => {
    ...
    // Run Sample Tests
    context.subscriptions.push(vscode.commands.registerCommand('dsaRoom.runTests', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showErrorMessage('No active editor');
            return;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('dsaRoom.generateTests', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showErrorMessage('No active editor');
            return;
    ...
    // Submit Solution
    context.subscriptions.push(vscode.commands.registerCommand('dsaRoom.submitSolution', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showErrorMessage('No active editor');
            return;
...
function startContest(context) {
    ...
    showWebview(context);
    vscode.window.showInformationMessage(`Contest started! Duration: ${Math.floor(currentRoom.duration / 60000)} minutes`);
    // Set timer to end contest
    setTimeout(() => {
        endContest(context);
    }, currentRoom.duration);
}
function endContest(context) {
    if (!currentRoom)
        return;
    currentRoom.status = 'ended';
    vscode.window.showInformationMessage('Contest ended! Calculating results...', 'View Results').then(action => {
...
function showResults(context) {
    if (!currentPanel) {
        currentPanel = vscode.window.createWebviewPanel('dsaResults', 'Contest Results', vscode.ViewColumn.Two, {
            enableScripts: true,
